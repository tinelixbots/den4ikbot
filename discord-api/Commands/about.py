...
async def editEmbed(ctx, bot, config, links, language, disnake, translator, python_version, uptime):
    ...
    if(len(links['website']) > 0 and len(links['support']) > 0 and len(links['repo']) > 0):
        ...
        msg_embed.add_field(
            translator.translate('embed_fields', 'about_linksf', language), translator.translate('embed_fields', 'about_linksv3', language).format(links['invite'], links['repo']), inline=True
        )
